...
import StoryApi from './createstoryapi';

const Create = ({ handleCloseCreate, handleGetAll }) => {
  ...
  const handleAddSlide = () => {
    ...
    const addon = [...slides.SlideData, { Heading: '', Description: '', Image: '' }];
  ...
  const handleDeleteSlide = (index, e) => {
    ...
    if (slides.SlideData.length > 1) {
      ...
      setCurrentSlideIndex(currentSlideIndex > index ? currentSlideIndex - 1 : 0);
    }
  } 
  ...
  const handlePost = () => {
    ...
    if (!currentSlide.Heading || !currentSlide.Description || !currentSlide.Image) {
  ...
  return (
    ...
    >
      ...
      <p style={{ marginLeft: '70%' }}>Add upto 6 slides</p>

      <div style={{ display: 'flex', flexDirection: 'row', height: '2rem', marginLeft: '0.5rem' }}>
        ...
        {slides.SlideData.length < 6 && (
          <button
            type="button"
            className="slides"
            style={{ marginLeft: '1rem', background: 'white', borderRadius: '0.5rem', fontWeight: 'bold', fontFamily: 'sans-serif', width: '4rem', width: '4rem',height:'2.5rem',borderStyle:'initial' }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', padding: '1rem 1rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', marginBottom: '0.1333rem' }}>
...
<br/>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', marginTop: '0.5rem', marginLeft: '2rem' }}>
        <button type="button" style={{ background: '#7EFF73', color: 'white', height: '1.7rem', width: '4.5rem', borderRadius: '1rem' }}
        onClick={handlePrevious}
        disabled={currentSlideIndex === 0}
      >
          Previous
        </button>
        <button type="button" style={{ background: '#73ABFF', color: 'white',height: '1.7rem', width: '4.5rem', borderRadius: '1rem', marginLeft: '1rem',borderStyle:'none' }}
        onClick={handleNext}
        disabled={currentSlideIndex === slides.SlideData.length - 1}
      >
          Next
        </button>
        <button type="button" style={{ background: ' #FF7373', color: 'white',height: '1.7rem', width: '4.5rem', borderRadius: '1rem', marginLeft: '10rem',borderStyle:'none'}} onClick={() => handlePost()}>
          Post
        </button>
      </div>
    </div>
